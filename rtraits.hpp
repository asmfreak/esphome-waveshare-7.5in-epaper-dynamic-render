...
#define TRAIT_CONSTRUCTOR_FUNCTION(name, ret, args, cv)                         \
    tpimpl::FunctionTypeStruct<                                                 \
        decltype(FunctionTable::name),                                          \
        decltype(&T::name),                                                     \
        &T::name>
...
#define TRAIT_INTERNAL(name, TABLE, CONSTRUCTOR, FDEFINITION)                  \
    class name {                                                               \
        ...
        const FunctionTable *_ftable = nullptr;                                \
                                                                               \
        template <typename T>                                                  \
        using ftable = tpimpl::FunctionTableInstance<                  \
            T,                                                                  \
            name,                                                               \
            FunctionTable,                                                      \
    ...
    public:                                                                    \
        ...
        template <typename T>                                                   \
        friend T* trait_cast(name& d);                                          \
    };                                                                         \
    template <typename T>                                                       \
    T* trait_cast(name& d){                                                     \
        if(name::ftable<T>::table != d._ftable){ return nullptr; }               \
        return reinterpret_cast<T *>(d._p);                                      \
    }                                                                           \
    template <typename T>                                                       \
    T* trait_cast(const name& d){                                               \
        if(name::ftable<T>::table != d._ftable){ return nullptr; }               \
        return reinterpret_cast<const T *>(d._p);                                \
    }                                                                           \
    ...
    name##_Owning make##name(Args... args){                                     \
...
#define Trait4(name, f1, f2, f3, f4)                                            \
    TRAIT_INTERNAL(name, TRAIT_FTABLE_FUNCTION f1; TRAIT_FTABLE_FUNCTION f2;    \
                    TRAIT_FTABLE_FUNCTION f3; TRAIT_FTABLE_FUNCTION f4;         \
    ,                                                                           \
    TRAIT_EXPAND(TRAIT_CONSTRUCTOR_FUNCTION f1,                                 \
                TRAIT_CONSTRUCTOR_FUNCTION f2,                                  \
                TRAIT_CONSTRUCTOR_FUNCTION f3,                                  \
                TRAIT_CONSTRUCTOR_FUNCTION f4),                                 \
    TRAIT_FUNCTION_DEFINITION f1 TRAIT_FUNCTION_DEFINITION f2                   \
    TRAIT_FUNCTION_DEFINITION f3 TRAIT_FUNCTION_DEFINITION f4)
